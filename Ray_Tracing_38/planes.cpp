...
bool triangle::is_interior(double _a, double _b, hit_record_gpu_openmp& _rec) const
{
	return _a > 0 && _b > 0 && _a + _b < 1;
}
...
bool circle::is_interior(double _a, double _b, hit_record_gpu_openmp& _rec) const
{
	return std::sqrt((_a - 0.5) * (_a - 0.5) + (_b - 0.5) * (_b - 0.5)) < 0.5;
}